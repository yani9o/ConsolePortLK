...
for name, script in pairs({
	...
	Input = [[
		...
		control:CallMethod('Filter', tostring(input))
	]],
	...
	SetTarget = [[
		...
		if self:GetAttribute('ghostMode') then
			control:CallMethod('HideBindings', unit, true)
			control:RunAttribute('DisplayBindings', true)
			control:RunAttribute('Wipe')
		else
			control:RunAttribute('Wipe')
			control:CallMethod('HideBindings', unit)
		end
	...
	AssignUnits = [[
		control:RunAttribute('CreateBindings')
	...
	Refresh = [[
		pool, onDown = ...
		if not onDown then
			control:RunAttribute('SetTarget')
			return
		end

		self:SetAttribute('macrotext', nil)
		control:RunAttribute('Wipe')
		control:RunAttribute('SetBindings', pool)
		control:RunAttribute('UpdateUnits', pool)
		control:RunAttribute('SortUnits')
		control:RunAttribute('AssignUnits', pool)
		control:RunAttribute('DisplayBindings')
	]],
		...
		if stack then
			for i, frame in pairs(stack) do
				if frame:IsProtected() then
					current = frame
					control:RunAttribute('UpdateFrames')
				end
			end
		end
	]],
	...
	UpdateUnits = [[
		...
		elseif pool == 'plates' or pool == 'tab' then
			control:RunAttribute('UpdatePlates')
		end
	]],
...
for name, script in pairs({
	PreClick = [[
		...
		if button == 'LeftButton' then
			control:RunAttribute('Refresh', 'frames', down)

		-- Nameplates
		elseif button == 'RightButton' then
			control:RunAttribute('Refresh', 'plates', down)
		
		-- Combined nameplates/nearest
		elseif button == 'MiddleButton' then
			control:RunAttribute('Refresh', 'tab', down)
			if down then
				defaultToTab = true
				control:CallMethod('HighlightTarget', false)
			end
		end
	]],
...
for name, script in pairs({
	OnClick = [[
		if down then
			control:RunAttribute('Input', button)
		end
	]],
...
function EM:OnNewBindings(...)
	...
	self:Execute([[control:RunAttribute('OnNewSettings')]])
...
function EM:RefreshUnitFrames(current)
	...
	if stack then
		for i, frame in pairs(stack) do
			if frame:IsProtected() then  -- and not frame:IsForbidden()
...
local numChildren, lastChildren = 0,0

local function findFrame(...)
	for i = lastChildren + 1, numChildren do
		local child = select(i, ...)
		if(SecureButton_GetUnit(child)) then
			EM:AddFrameForUnit(child, SecureButton_GetUnit(child))
		end
	end
end

function EM:OnUpdate(elapsed) 
	numChildren = UIParent:GetNumChildren()
	if lastChildren ~= numChildren then 
		findFrame(UIParent:GetChildren())
